...
export default function SessionRequestMessage({ session, isReceiver, onAccept, onDecline, user, selectedUser }) {
  ...
  const isCombine = !session.skill_request_id;
  ...
  // Message for combine request
  const combineMessageText = (
    <div className="flex flex-col items-start gap-3">
      <div className="flex items-center gap-2">
        <svg className="w-5 h-5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <span className="text-base font-medium text-purple-300">Skill Combination Request</span>
      </div>
      <div className="flex flex-col gap-2 bg-[#251a34] p-3 rounded-lg w-full">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-300">This is a request to combine your skills for a unique session!</span>
        </div>
        {session.mode && (
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span className="text-sm text-gray-300">Mode: <span className="text-purple-400 font-semibold">{session.mode === 'online' ? 'Online' : 'In Person'}</span></span>
          </div>
        )}
        {session.location && session.mode === 'in_person' && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-300">Location: <span className="text-purple-400 font-semibold">{session.location}</span></span>
          </div>
        )}
        {session.notes && (
          <div className="flex items-start gap-2">
            <svg className="w-4 h-4 text-purple-400 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <span className="text-sm text-gray-300">Notes: <span className="text-purple-400">{session.notes}</span></span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className={`flex ${senderIsCurrentUser ? 'justify-end' : 'justify-start'} items-start gap-2`}>
      ...
      <div className="flex flex-col max-w-[60%]">
        ...
        <div className={`text-center flex flex-col justify-center rounded-2xl px-4 py-3 ${senderIsCurrentUser ? 'bg-blue-500 text-white' : 'bg-[#232e39] text-white'}`}>
          {isCombine ? combineMessageText : messageText}
          {isReceiver && session.status === "pending" && (
            <div className="flex mt-4 justify-center gap-10 w-full">
              <Button
                className={isCombine ? "bg-purple-600 hover:bg-purple-700 text-white px-10 py-1 rounded-lg font-medium" : "bg-blue-500 text-white px-10 py-1 rounded-lg font-medium"}
                onClick={() => onAccept(session.id)}
              >
                {isCombine ? "Accept Combination" : "Accept"}
              </Button>
              <Button
                variant="outline"
                className="border-white cursor-pointer text-white hover:text-white bg-transparent rounded-lg font-medium px-10 transition hover:bg-white/10"
